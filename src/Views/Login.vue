      ...
      <div class="login-container-body">
        ...
        <form class="login-form">
          <input id="userUsername"
                 class="login-form_input-group_field"
                 placeholder="Enter Username"/>
          <input id="userPassword"
                 class="login-form_input-group_field"
                 placeholder="Enter Password"/>
          <button class="login-form_button">
            Login!
          </button>
        </form>
      </div>
...
export default {
  ...
  methods:{
    async ImageGetter(){
      this.sidebarPic = this.loadingScreen;
      const image = await fetch("https://picsum.photos/1000/1200");
      const imageJson = image.url;
      this.sidebarPic = imageJson;
    }
  }
}
</script>
